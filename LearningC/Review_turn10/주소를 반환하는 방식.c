...
int* sumFunc(int a, int b); // int*형의 sum() 함수 선언

int main()
{
	int* resp;			// 합을 구하는 sumFunc 함수의 반환값(주소)의 포인턴 변수 resp 

	resp = sumFunc(30, 50); // 주소값을 반환받기 위한 포인터 변수 resp 초기화
...
//	static int sum; // 함수 호출 이후 메모리 공간 회수되면(staitc 선언X), 아래 경고 메시지 발생 
	int sum;		// warning: address of stack memory associated with local variable 'sum' returned [-Wreturn-stack-address]
					...
					// 데이터(댕글링 포인터)를 가리키게 되어 심각한 런타임 오류나 잘못된 값을 유발할 수 있음 -> return 값을 sum으로 변경하거나, static 선언
					// 
					// 동적할당(malloc)해서 사용하거나, 외부메모리에 포인터로 인자를 가리켜 사용방법도 가능
    				...
    				// 		return sum;
					// }
					// 2) 외부 메모리 사용: 함수 외부에서 선언된 변수의 포인터를 인자로 전달받아 값을 저장합니다. -> Call by Reference
	sum = a + b;
